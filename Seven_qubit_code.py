...
# # #####
# p_error = 0.001 #Error probability
...
# #####
stab1Z=QuantumCircuit(5, name='M1') #ZZIIIZZ
for i in range(0,stab1Z.num_qubits-1):
    stab1Z.cz(4,i)
stab2Z=QuantumCircuit(5, name='M2') #ZIZIZIZ
for i in range(0,stab2Z.num_qubits-1):
    stab2Z.cz(4,i)
stab3Z=QuantumCircuit(5, name='M3') #ZIIIZZZ
...
qc_3qx.append(stab1Z,[0,1,5,6,7])
qc_3qx.append(stab2Z,[0,2,4,6,8])
qc_3qx.append(stab3Z,[0,3,4,5,9])
qc_3qx.append(stab1X,[0,1,5,6,10])
qc_3qx.append(stab2X,[0,2,4,6,11])
qc_3qx.append(stab3X,[0,3,4,5,12])
for i in range(7,13):
        qc_3qx.h(i)
qc_3qx.barrier()
...
plot_histogram(counts)
plt.show()
# counts=execute(qc_3qx, aer_sim, optimization_level=0, noise_model= noise_model, shots=10000).result().get_counts()
# plot_histogram(counts)
# plt.show()
